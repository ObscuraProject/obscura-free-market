...
func TaskFreezeHistoricOldFrozenTransactions() {
	it := FindPre2025FrozenTransactions()
	util.Log.Debug("[Task] [TaskFreezeHistoricOldFrozenTransactions] # of items:", len(it))
	for _, t := range it {
		// t.RefreshTransactionStatus()
		t.FreezeHistoric("Status has been changed as FROZEN_HISTORIC", "")
	}
}
