...
import (
	...
	"github.com/ObscuraProject/obscura-free-market/modules/apis"
)
...
type TransactionStatus struct {
	...
	PaymentReceiptUuid string    `json:"-" sql:"index"`
...
func (t Transaction) ReleaseHistoric(comment, userUuid string) error {
	t.SetTransactionStatus(
		"RELEASED_HISTORIC",
		t.CurrentAmountPaid(),
		comment,
...
func (t Transaction) FreezeHistoric(comment, userUuid string) error {
	t.SetTransactionStatus(
		"FROZEN_HISTORIC",
		t.CurrentAmountPaid(),
		comment,
...
func (t Transaction) IsFrozenHistoric() bool {
	return t.CurrentPaymentStatus() == "FROZEN_HISTORIC"
}
...
func FindPre2025FrozenTransactions() []Transaction {
	date := time.Date(2025, 9, 1, 0, 0, 0, 0, time.UTC)

	frozenTransactions := findTransactionsWithCurrentStatusAndTimestamp(
		"FROZEN",
		date,
	)
