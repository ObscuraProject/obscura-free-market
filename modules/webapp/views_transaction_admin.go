...
func (c *Context) AdminTransactionFreezeHistoric(w web.ResponseWriter, r *web.Request) {
	transaction, _ := FindTransactionByUuid(r.PathParams["transaction"])
	if transaction == nil {
		http.NotFound(w, r.Request)
		return
	}
	err := transaction.FreezeHistoric("Transaction frozen by staff", "")
	if err != nil {
		c.Error = err.Error()
	}
...
func (c *Context) AdminTransactionReleaseHistoric(w web.ResponseWriter, r *web.Request) {
	transaction, _ := FindTransactionByUuid(r.PathParams["transaction"])
	if transaction == nil {
		http.NotFound(w, r.Request)
		return
	}
	err := transaction.ReleaseHistoric("", "")
	if err != nil {
		c.Error = err.Error()
	}
	http.Redirect(w, r.Request, "/payments/"+transaction.Uuid, http.StatusFound)
